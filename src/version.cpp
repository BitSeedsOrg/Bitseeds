...
const std::string CLIENT_NAME("BitSeeds");
...
#define BUILD_DESC_FROM_COMMIT(maj,min,rev,commit) \
    "XSEED-v" DO_STRINGIZE(maj) "." DO_STRINGIZE(min) "." DO_STRINGIZE(rev) "." DO_STRINGIZE(build) "-g" commit

#define BUILD_DESC_FROM_UNKNOWN(maj,min,rev) \
    "XSEED-v" DO_STRINGIZE(maj) "." DO_STRINGIZE(min) "." DO_STRINGIZE(rev) "." DO_STRINGIZE(build) "-unk"
