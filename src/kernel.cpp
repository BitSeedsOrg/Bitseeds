...
// Copyright (c) 2014 The BitSeeds developers
...
int64_t GetWeight(int64_t nIntervalBeginning, int64_t nIntervalEnd, int64_t nHeight)
{
	...
	//printf ("GetStakeMinAge(): %s\n",GetStakeMinAge(nHeight));
    return min(nIntervalEnd - nIntervalBeginning - GetStakeMinAge(nHeight), (int64_t)nStakeMaxAge);
}
...
static bool GetKernelStakeModifier(uint256 hashBlockFrom, uint64_t& nStakeModifier, int& nStakeModifierHeight, int64_t& nStakeModifierTime, bool fPrintProofOfStake)
{
	...
	if (fDebug && GetBoolArg("-printcoinstake"))
		printf("GetKernelStakeModifier() : nStakeModifierTime=%d < pindexFrom->GetBlockTime() + nStakeModifierSelectionInterval = %d \n", nStakeModifierTime, pindexFrom->GetBlockTime() + nStakeModifierSelectionInterval);
    while (nStakeModifierTime < pindexFrom->GetBlockTime() + nStakeModifierSelectionInterval)
    {
		if (fDebug && GetBoolArg("-printcoinstake"))
			printf("GetKernelStakeModifier() : nStakeModifierTime=%d < this=%d\n", nStakeModifierTime,pindexFrom->GetBlockTime() + nStakeModifierSelectionInterval);
        if (!pindex->pnext)
        {   // reached best block; may happen if node is behind on block chain
			if (fDebug && GetBoolArg("-printcoinstake"))
				printf("GetKernelStakeModifier() : reached best block");
    ...
    nStakeModifier = pindex->nStakeModifier;
	if (fDebug && GetBoolArg("-printcoinstake"))
		printf("GetKernelStakeModifier() : %d\n", nStakeModifier);
    return true;
}
...
static bool CheckStakeKernelHashV1(unsigned int nBits, const CBlock& blockFrom, unsigned int nTxPrevOffset, const CTransaction& txPrev, const COutPoint& prevout, unsigned int nTimeTx, uint256& hashProofOfStake, uint256& targetProofOfStake, bool fPrintProofOfStake)
{
    ...
    if (nTimeBlockFrom + GetStakeMinAge(pindexBest->nHeight) > nTimeTx) // Min age requirement
    ...
    CBigNum bnCoinDayWeight = CBigNum(nValueIn) * GetWeight((int64_t)txPrev.nTime, (int64_t)nTimeTx, GetStakeMinAge(pindexBest->nHeight)) / COIN / (24 * 60 * 60);
    ...
    if (!GetKernelStakeModifier(hashBlockFrom, nStakeModifier, nStakeModifierHeight, nStakeModifierTime, fPrintProofOfStake))
        return false;
	if (fDebug && GetBoolArg("-printcoinstake"))
        printf("CheckStakeKernelHash() : Stake modifier OK...\n");
...
static bool CheckStakeKernelHashV2(CBlockIndex* pindexPrev, unsigned int nBits, unsigned int nTimeBlockFrom, const CTransaction& txPrev, const COutPoint& prevout, unsigned int nTimeTx, uint256& hashProofOfStake, uint256& targetProofOfStake, bool fPrintProofOfStake)
{
    ...
    if (fDebug)
    {
        ...
        printf("CheckStakeKernelHash() : check modifier=0x%016"PRIx64" nTimeBlockFrom=%u nTimeTxPrev=%u nPrevout=%u nTimeTx=%u hashProof=%s bnTarget=%s\n",
            nStakeModifier,
    ...
    if (CBigNum(hashProofOfStake) > bnTarget){
    	if (fDebug) printf("CheckStakeKernelHash() : under target");
        return false;
    }
...
bool CheckStakeKernelHash(CBlockIndex* pindexPrev, unsigned int nBits, const CBlock& blockFrom, unsigned int nTxPrevOffset, const CTransaction& txPrev, const COutPoint& prevout, unsigned int nTimeTx, uint256& hashProofOfStake, uint256& targetProofOfStake, bool fPrintProofOfStake)
{
    if (IsProtocolV2(pindexPrev->nHeight+1)){
    	if (fDebug) printf ("CheckStakeKernelHash(): V2\n");
    	...
    	if (fDebug) printf ("CheckStakeKernelHash(): V1\n");
