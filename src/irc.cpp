...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #BitSeedsNewTEST2\r");
            Send(hSocket, "WHO #BitSeedsNewTEST2\r");
        } else {
            // randomly join #BitSeedsNew00-#BitSeedsNew05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #BitSeedsNew%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #BitSeedsNew%02d\r", channel_number).c_str());
        }
